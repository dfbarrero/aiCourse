...
\begin{frame}{Search problems}{Travel Salesman Problem (TSP)}
       \begin{columns}
 	       \column{.50\textwidth}
                \begin{block}{TSP formulation}
                    A travel salesman must visit a set of cities only one time each. Find the shortest route.
                \end{block}
...
\begin{frame}{Study case}{Case study II: 9\textsuperscript{th} Global Trajectory Optimization Competition (I)}
    ...
    \end{itemize}
    \href{https://www.youtube.com/watch?time_continue=86&v=zvxZx-QnqQ0}{(Video)}
